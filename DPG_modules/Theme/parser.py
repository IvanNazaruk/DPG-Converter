...
with open("themes.toml", "rb") as f:
    ...
    for theme in data:
        ...
        for style_name in list(styles.keys()):
            ...
            style_name = rename.get(style_name)
        ...
        for color_name in list(colors.keys()):
            ...
            color_name = rename.get(color_name)
            if color_name is None:
                raise ValueError(f"Unknown color type: {color_name}")
            output_string += f"    {color_name}: {color_value},\n"
        output_string += '}\n'

        output_string += 'clear_color = colors.get(dpg.mvThemeCol_WindowBg, (0, 0, 0))\n'


        file_name = name.lower().replace(' ', '_')
        file_name = list(
            filter(
                lambda char: char in f"{string.ascii_lowercase}_", file_name
            )
        )

        file_name = ''.join(file_name)
        with open(os.path.join('themes', f"{file_name}.py"), "w") as file:
