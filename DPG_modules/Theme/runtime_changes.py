...
from ..Addons import dpg_callback
from ..Addons.dpg_handler import SubscriptionTag
...
def update_theme_callback(progress: float):
    CurrentTheme.changing_progress = progress
    ...
    for dpg_theme, callback in list(colors_theme_subscribers.values()):
        ...
        if value is not None:
            try:
                ...
                logging.error(f"Error in {callback}:", exc_info=True)
            continue
