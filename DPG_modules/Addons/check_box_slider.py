...
from .. import Theme as dpg_theme
from ..Animations import PosAnimation, TextColorAnimation
from ..Animations.dpg.position import PosValue
                ...
                with dpg.group(parent=theme_group) as group:
                    with dpg.drawlist(width=self.WIDTH, height=self.HEIGHT, parent=group) as drawlist:
                        ...
                        self.pos_animation = CheckBoxAnimation(foreground_circle, [circle_radius, circle_radius]) \
                            .add_point([self.WIDTH - circle_radius, circle_radius], self.ANIMATION_DURATION,
                                       (.55, -0.01, .4, 1))
...
class ThemeCheckBoxSlider(_CheckBoxSlider):
    ...
    def _update_frame_padding(self, _):
        ...
        for horizontal_spacer, vertical_spacer in zip(self.horizontal_frame_padding_objects,
                                                      self.vertical_frame_padding_objects):
            try:
    ...
    def create(self, value: bool = True) -> 'Self':
        ...
        if self.fg_color_theme_change_tag:
            dpg_theme.unsubscribe_color_theme_change(self.fg_color_theme_change_tag)
        self.bg_color_theme_change_tag = dpg_theme.subscribe_color_theme_change(dpg.mvThemeCol_FrameBg,
                                                                                self._update_background_color)
        self.fg_color_theme_change_tag = dpg_theme.subscribe_color_theme_change(dpg.mvThemeCol_Text,
        ...
        self.fp_style_theme_change_tag = dpg_theme.subscribe_style_theme_change(dpg.mvStyleVar_FramePadding,
                                                                                self._update_frame_padding)
        return self
