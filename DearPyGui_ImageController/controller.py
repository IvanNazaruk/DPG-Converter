...
ENABLE_LOADING = threading.Event()
ENABLE_LOADING.set()
            ...
            try:
                ...
                for image_viewer in self.subscribers.values():
                    try:
                        ...
                        pass
                        # traceback.print_exc()
            except RuntimeError:  # RuntimeError: dictionary changed size during iteration
                continue
            break
        if self.image:
...
class ImageUnloaderWorker:
    ...
    def loop(self):
        while True:
            ...
            for image_controller in self.queue.copy():
                ENABLE_LOADING.wait()
    ...
    def load(image_controller: ImageController):
        ...
        try:
            image_controller.load(
                tools.image_to_dpg_texture(
                    tools.resize_image_to_fit_screen_resolution(image_controller.image)
                )
            )
