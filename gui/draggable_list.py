...
import fonts
from DearPyGui_Addons import dpg_handler, get_dpg_window_by_object, get_local_mouse_pos, GetNowCursorType
from gui.load_window import load_window_tqdm
from settings import FullWindowScrollBar

cell_border_size = 3
cell_size = fonts.font_size * 2 + int(cell_border_size * 1.5)


class DraggableListCell(ABC):
    ...
    def __init__(self, image_list: DraggableList):
...
DraggableListCellType = TypeVar("DraggableListCellType", bound=DraggableListCell)
...
class DraggableList:
    ...
    all_cells: list[DraggableListCellType]
    is_now_selected: bool = False
    selected_cell: DraggableListCellType | None = None
    ...
    def remove_all(self):
        ...
        cell: DraggableListCellType
        for cell in load_window_tqdm(saved_all_cells):
            cell.delete()
        del saved_all_cells
        self.update_settings()

    def remove(self, cell: DraggableListCellType | int):
        index: int = cell
    ...
    def mouse_down(self):
        ...
        if not self.is_now_selected:
            ...
            if get_dpg_window_by_object(self.window) != get_dpg_window_by_object(dpg.get_active_window()):
                return
